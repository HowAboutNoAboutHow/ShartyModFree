(function () {
    ...
    if (!newFeaturesShown) {
      ...
      const message = `
        <strong style="font-size:20px;display:block;">Updates in ShartyMod:</strong><br>
    ...
    nameOverlay.className = 'sharty-overlay';
    ...
    const overlay = document.createElement('div');
    overlay.className = 'Sharty-overlay';

    const ShartyTitle = document.createElement('h3');
    ShartyTitle.className = 'Sharty-title';

    function updateModuleList() {

        moduleList.innerHTML = '';
        moduleList.className = 'sharty-module-list';
        ...
        let controls = [
            [ '[G] Aimbot', state.isAimBotEnabled, state.isAimBotEnabled ? 'ON' : 'OFF' ],
            [ '[J] Aim at Downed', state.isAimAtKnockedOutEnabled, state.isAimAtKnockedOutEnabled ? 'ON' : 'OFF' ],
            [ '[H] Auto Quick-switch', qs, qs ? 'ON' : 'OFF' ],
            // [ '[I] Lead Indicator', state.isAimBotEnabled, state.isAimBotEnabled ? 'ON' : 'OFF' ],
        ...
        controls.forEach((control, index) => {
            ...
            line.className = 'Sharty-control';
            line.style.color = "#FFFFFF";
            line.style.fontFamily = '"Trebuchet MS", sans-serif';
            line.style.opacity = isEnabled ? 1 : 0.5;
            line.style.marginBottom = (index === controls.length - 1) ? '0px' : '3px'; // No margin for last item
            ...
            // overlay.border = "10px solid #2f4032";
    ...
    document.querySelector('#ui-top-left').insertBefore(ShartyTitle, document.querySelector('#ui-top-left').firstChild);
    ...
    */
    function get_pred_pos(X1, Y1, v_X, v_Y, v_P){
        //applying our formula to solve for a solution time
    ...
    function calculatePredictedPosForShoot(enemy, curPlayer) {
        ...
        if (state.lastFrames[enemy.__id].length > 30)state.lastFrames[enemy.__id].shift();

        const deltaTime = (dateNow - state.lastFrames[enemy.__id][0][0]) / 1000; // t since last frame
        const enemyVelocity = {
        ...
        let bulletSpeed;
        if (!bullet)bulletSpeed = 1000;
        else bulletSpeed = bullet.speed;
        ...
        var predictedPos = get_pred_pos(dx,dy,vex,vey,vb);
        predictedPos.x += curPlayerPos._x;
        predictedPos.y += curPlayerPos._y;
...
//         }
        ...
        initCounters() {
            // Create a single container for all counters
    ...
    window.GameMod = new GameMod(); // AlguienClient
...
`;
    ...
    function keybinds(){
        window.document.addEventListener('keyup', function (event) {
            ...
            const validKeys = ['Z', 'P', 'O', 'K', 'H', 'N', 'U', 'G', 'J', 'Y','I'];
            ...
            switch (String.fromCharCode(event.keyCode)) {
                ...
                case validKeys[4]:
                    qs = !qs; break;
                case validKeys[5]:
                    console.log(`🔀 Toggling ceilings: ${state.areCeilingsRemoved}`); // Debug log
    ...
    let rc = true;
    state.areCeilingsRemoved = false; // Default: Ceilings visible
    ...
    window.initGameControls = function(gameControls){
        ...
        // autoMelee
        if ((window.game.touch.shotDetected || window.game.inputBinds.isBindDown(inputCommands.Fire)) && window.aimTouchMoveDir) {
    ...
    function overrideMousePos() {
        Object.defineProperty(window.game.input.mousePos, 'x', {
            get() {
                ...
                // Aimbot Logic
                if (state.isAimBotEnabled && state.focusedEnemy && window.game.inputBinds.isBindDown(inputCommands.Fire)) {
                    const enemy = findClosestEnemy();
                    if (enemy) {
                        const predictedPos = calculatePredictedPosForShoot(enemy, me);
                        if (predictedPos) {
                            spinPaused = true; // Temporarily pause SpinBot
                            clearTimeout(spinResumeTimeout); // Reset any previous timers
                            spinResumeTimeout = setTimeout(() => {
                                spinPaused = false; // Resume SpinBot after delay
                            }, 50); // 250ms delay before resuming
                            return predictedPos.x;
                        }
                    }
                }

                if (window.game.inputBinds.isBindDown(inputCommands.EmoteMenu) || window.game.inputBinds.isBindPressed(inputCommands.EmoteMenu)) {
                return this._x;
                }
                ...
                // Spinbot Logic
                if (state.isSpinBotEnabled && !(window.game.inputBinds.isBindDown(inputCommands.Fire)) && !spinPaused) {
                    spinAngle = (spinAngle + spinSpeed) % 360; // Keep angle between 0-360
    ...
    function findClosestEnemy() {
        const players = window.game.playerBarn.playerPool.pool;
        const me = window.game.activePlayer;
        const meTeam = getTeam(me); // get the players team
        let closestEnemy = null;
        let minDistance = Infinity;

        players.forEach((player) => {
            if (!player.active || player.netData.dead || player.__id === me.__id) return;
            if (getTeam(player) === meTeam) return; // ignore teammates
            if (player.layer !== me.layer) return; // Ignore players on a different layer
            if (!state.isAimAtKnockedOutEnabled && player.downed) return; // Ignore knocked-out players if setting is disabled
            const screenPlayerPos = window.game.camera.pointToScreen({x: player.pos._x, y: player.pos._y});
    ...
    function visibleNames(){
        ...
        pool.forEach(player => {
            Object.defineProperty(player.nameText, 'visible', {
                get() {
                    if (!vn) return false; // Hide if vn is false
        ...
        try{lineDrawer.clear();}
            ...
            if (state.isNadeDrawerEnabled){
                ...
                Object.values(window.game.objectCreator.idToObj)
                    ...
                    // Create gradient effect using multiple rings
                    let numRings = 1; // More rings = smoother gradient
                    for (let i = numRings; i >= 1; i--) {
                        let alpha = i / numRings * 0.3; // More center = more opacity
            ...
            if (state.isNadeDrawerEnabled){
                ...
                Object.values(window.game.objectCreator.idToObj)
                    ...
                    nadeDrawer.lineStyle(1, 0xff0000, 0.8); // (thickness, color, opacity)
                ...
                curBullet,
                 ...
                 opacity = 0.3,
                    ...
                    if(curBullet) {
                        ...
                        let atan;
                        if (acPlayer == me && ((!window.lastAimPos) || (window.lastAimPos && ! (window.game.touch.shotDetected || window.game.inputBinds.isBindDown(inputCommands.Fire))))) {
                            atan = Math.atan2(
                                window.game.input.mousePos._y - window.innerHeight / 2,
                                window.game.input.mousePos._x - window.innerWidth / 2,
                            );
                        }else if(acPlayer == me && (window.lastAimPos) && (window.game.touch.shotDetected || window.game.inputBinds.isBindDown(inputCommands.Fire))) {
                            const playerPointToScreen = window.game.camera.pointToScreen({x: acPlayer.pos._x, y: acPlayer.pos._y});
                            atan = Math.atan2(
            ...
            try {
                ...
                if (curWeap.ammo !== ammo[curWeapIdx]?.ammo) {
                    ...
                    if (
                        ...
                        // Switch to the other weapon if conditions are met
    ...
    function grenadeTimer() {
       ...
       try {
           ...
           const time = 4; // Duration for the grenade timer
           if (elapsed > time) {
               showing = false;
           }
           if (!showing) {
    ...
    function isThrowingGrenade() {
        ...
        // Check if the active weapon is a grenade
